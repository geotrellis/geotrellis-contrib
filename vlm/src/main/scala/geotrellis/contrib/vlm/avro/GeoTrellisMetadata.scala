...
object GeoTrellisMetadata {
  def apply(rasterSource: RasterMetadata, base: Map[String, String]): GeoTrellisMetadata =
    GeoTrellisMetadata(rasterSource.name, rasterSource.crs, rasterSource.bandCount, rasterSource.cellType, rasterSource.gridExtent, rasterSource.resolutions, base)

  def apply[F[_]: Monad](rasterSource: RasterSourceMetadataF[F], base: F[Map[String, String]]): F[GeoTrellisMetadata] =
