...
case class GeoTiffResampleRasterSource(
  ...
  override lazy val layerGridExtent: RasterExtent = resampleGrid(tiff.rasterExtent)
  lazy val resolutions: List[RasterExtent] = {
    val ratio = layerGridExtent.cellSize.resolution / tiff.rasterExtent.cellSize.resolution
    layerGridExtent :: tiff.overviews.map { ovr =>
  ...
  @transient protected lazy val closestTiffOverview: GeoTiff[MultibandTile] =
    tiff.getClosestOverview(layerGridExtent.cellSize, strategy)
  ...
  def reproject(targetCRS: CRS, reprojectOptions: Reproject.Options, strategy: OverviewStrategy): GeoTiffReprojectRasterSource =
    new GeoTiffReprojectRasterSource(uri, targetCRS, reprojectOptions, strategy, targetCellType) {
      override lazy val rasterExtent: RasterExtent = reprojectOptions.targetRasterExtent match {
        ...
        case None => ReprojectRasterExtent(self.layerGridExtent, this.transform, this.reprojectOptions)
      }
    }
  ...
  override def readExtents(extents: Traversable[Extent], bands: Seq[Int]): Iterator[Raster[MultibandTile]] = {
    val targetPixelBounds = extents.map(layerGridExtent.gridBoundsFor(_))
  ...
  override def readBounds(bounds: Traversable[GridBounds], bands: Seq[Int]): Iterator[Raster[MultibandTile]] = {
    ...
    val windows = { for {
      ...
      val targetExtent = layerGridExtent.extentFor(targetPixelBounds)
