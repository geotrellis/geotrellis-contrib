...
class GDALRasterSummarySpec extends FunSpec with TestEnvironment with BetterRasterMatchers with GivenWhenThen {
  ...
  describe("Should collect GDALRasterSource RasterSummary correct") {
    ...
    it("should collect summary for a tiled to layout source GDAL") {
      ...
      val tiledRDD = sourceRDD.map(_.tileToLayout(layout, (_, sk) => sk, method))
  ...
  it("Should cleanup GDAL Datasets by the end of the loop (10 iterations)") {
    ...
    cfor(0)(_ < 11, _ + 1) { _ =>
      val reference = GDALRasterSource(inputPath).reproject(targetCRS, method = method).tileToLayout(layout, (_, sk) => sk, method)
