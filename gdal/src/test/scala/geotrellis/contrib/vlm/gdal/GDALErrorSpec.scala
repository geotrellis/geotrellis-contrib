...
class GDALErrorSpec extends FunSpec {
  ...
  val dataset: Int = GDALWarpOptions.EMPTY.datasetType
  ...
  describe("GDALErrors") {
    it("should return the ObjectNull error code") {
      val result = GDALWarp.get_data(token, dataset, 1, sourceWindow, destWindow, 42, 1, buffer)

      assert(math.abs(result) == 10)
    }

    it("should return the IllegalArg error code") {
      val result = GDALWarp.get_data(token, dataset, 1, sourceWindow, destWindow, 1, 1, buffer)
