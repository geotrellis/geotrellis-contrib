...
class GDALDataPathSpec extends FunSpec with Matchers {
  ...
  describe("Formatting the given uris") {
    describe("http") {
      it("http url") {
        ...
        GDALDataPath.parse(url).value should be (expectedPath)
      }
    ...
    describe("file") {
      it("file uri") {
        ...
        GDALDataPath.parse(uri).value should be (expectedPath)
      }
      ...
      it("force zip+file with ! uri") {
        ...
        println(s"GDALDataPath.parse(uri): ${GDALDataPath.parse(uri)}")
  ...
  describe("Formatting VSI paths") {
    it("should parse a VSI path") {
      ...
      GDALDataPath.parse(filePath).value should be (filePath)
    }
  ...
  describe("Formatting the given uris - edge cases") {
    it("should parse a path with uncommon characters") {
      ...
      GDALDataPath.parse(uri, None).value should be (expectedPath)
    }
    ...
    it("should parse a targeted compressed file with a differenct delimiter") {
      ...
      GDALDataPath.parse(uri, Some("/")).value should be (expectedPath)
    }
  }
}
