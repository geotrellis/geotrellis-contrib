...
case class GDALDataset(token: Long) extends AnyVal {
  ...
  def readTile(gb: GridBounds = rasterExtent.gridBounds, band: Int, dataset: Int = GDALWarp.WARPED): Tile = {
    require(acceptableDatasets contains dataset)
  ...
  def readMultibandTile(gb: GridBounds = rasterExtent.gridBounds, bands: Seq[Int] = 1 to bandCount, dataset: Int = GDALWarp.WARPED): MultibandTile =
    MultibandTile(bands.map { readTile(gb, _, dataset) })

  def readMultibandRaster(gb: GridBounds = rasterExtent.gridBounds, bands: Seq[Int] = 1 to bandCount, dataset: Int = GDALWarp.WARPED): Raster[MultibandTile] =
